...
inline std::size_t getFixedPointWorkspaceSizeInBytes(const int m,
                                                     const int n,
                                                     const int k,
                                                     ...
                                                     const int max_mantissa_bit_count)
{
  ...
  constexpr std::size_t CONSTANT_SIZE = 128ULL * 1024ULL * 1024ULL;
  ...
  return static_cast<std::size_t>(std::ceil(emulation_workspace * MULTIPLIER)) + CONSTANT_SIZE;
}
...
template<typename T>
inline void gemmFp64EmulatedFixedPoint(BLASHandle<PlatformKind::CUDA>& handle,
                                       const char transa,
                                       const char transb,
                                       int m,
                                       int n,
                                       ...
                                       const BLASPolicy& policy)
{
  const std::size_t required_workspace_bytes =
      getFixedPointWorkspaceSizeInBytes(m, n, k, 1, EmuTypeTraits<T>::is_complex, CUDA_EMULATION_MANTISSA_CONTROL_FIXED,
  ...
  cublasErrorCheck(cublasLtMatmulDescCreate(&operation_desc, CUBLAS_COMPUTE_64F_EMULATED_FIXEDPOINT,
  ...
  cublasErrorCheck(cublasLtMatrixLayoutCreate(&a_desc, EmuTypeTraits<T>::data_type, rows_a, cols_a, lda),
                   "cublasLtMatrixLayoutCreate A failed!");
  cublasErrorCheck(cublasLtMatrixLayoutCreate(&b_desc, EmuTypeTraits<T>::data_type, rows_b, cols_b, ldb),
...
template<typename T>
inline void gemmBatchedFp64EmulatedFixedPoint(BLASHandle<PlatformKind::CUDA>& handle,
                                              const char transa,
                                              const char transb,
                                              int m,
                                              int n,
...
{
  const std::size_t required_workspace_bytes =
      getFixedPointWorkspaceSizeInBytes(m, n, k, batchCount, EmuTypeTraits<T>::is_complex,
                                        CUDA_EMULATION_MANTISSA_CONTROL_FIXED, policy.max_mantissa_bits);
  const std::size_t requested_workspace_bytes = std::max(policy.min_workspace_bytes, required_workspace_bytes);
  auto& lt_emulation_context                  = handle.ensureLtEmulationContext(requested_workspace_bytes);
